/**
 * Verify technical expert structure in MongoDB
...
async function verifyTechnicalExpertStructure() {
  try {
    console.log('🔍 Verifying Technical Expert Structure...\n');

    // 1. Check questionnaire
    console.log('1️⃣ Checking questionnaire...');
    const questionnaire = await Questionnaire.findOne({ key: 'technical-expert-v1' });
    if (questionnaire) {
      ...
      console.log('❌ Questionnaire technical-expert-v1 not found!');
    }

    // 2. Check questions
    console.log('\n2️⃣ Checking questions...');
    const questions = await Question.find({ questionnaireKey: 'technical-expert-v1' })
      .sort({ order: 1 })
      .select('code principle text.en order answerType');
    
    console.log(`✅ Found ${questions.length} technical expert questions:`);
    ...
    const responses = await Response.find({ questionnaireKey: 'technical-expert-v1' })
      .select('projectId userId role questionnaireKey answers status submittedAt')
      .populate('projectId', 'title')
      .populate('userId', 'name email');
    
    console.log(`✅ Found ${responses.length} technical-expert-v1 responses:`);
    ...
    // 4. Check scores
    console.log('\n4️⃣ Checking scores...');
    const scores = await Score.find({ questionnaireKey: 'technical-expert-v1' })
      .select('projectId userId role questionnaireKey overallScore principleScores')
      .populate('projectId', 'title')
      .populate('userId', 'name email');
    
    console.log(`✅ Found ${scores.length} technical-expert-v1 scores:`);
    if (scores.length > 0) {
    ...
    console.log(`   Questions: ${questions.length} (expected: 12)`);
    console.log(`   Responses: ${responses.length}`);
    console.log(`   Scores: ${scores.length}`);

    if (questionnaire && questions.length === 12) {
      console.log('\n✅ Technical Expert structure is correctly set up!');
    } else {
...
verifyTechnicalExpertStructure();
